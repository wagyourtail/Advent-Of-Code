...
c = {}
[a, b] = [a.split(",") for a in inp.split("\n")]
e = []
...
closest = e[0]
for i in e:
    if (abs(i[0]) + abs(i[1])) < (abs(closest[0]) + abs(closest[1])):
        closest = i
...
def part2(m, wid):
    ...
    for i in m:
        ...
        for _ in range(int(i[1:])):
            ...
            distance += 1
            if (px, py) in e:
                wires[wid][(px, py)] = distance

part2(a, 0)
part2(b, 1)
closest = wires[0][e[0]] + wires[1][e[0]]
for i in e:
    if wires[0][i] + wires[1][i] < closest:
        closest = wires[0][i] + wires[1][i]
